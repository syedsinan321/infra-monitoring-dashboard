...
import { KeyRound, Search, RefreshCw, ChevronUp, ChevronDown, ShieldCheck, ShieldX, ShieldAlert, Server, AlertTriangle, Upload, Copy, Check, Play, Terminal, ChevronRight } from 'lucide-react';
...
function TPMKeysPage() {
  ...
  useEffect(() => {
    if (authenticated) {
      // Load cached data immediately, then sync inventory in background
      loadData().then(() => loadData({ sync: true }));
    }
    ...
    try {
      ...
      const poll = async () => {
        try {
          ...
          if (status.target_count && collectTarget === 0) setCollectTarget(status.target_count);
          ...
          if (status.status === 'done') {
            setImportResult({
              success: true,
              importedCount: status.imported_count || 0,
            ...
            setImportResult({
              success: false,
              error: status.errors?.[0]?.error || 'Collection failed',
            });
          }
        } catch (err) {
          setCollecting(false);
  ...
  const filtered = useMemo(() => {
    ...
    if (filterDC !== 'all') {
      list = list.filter((t) => t.group === filterDC);
    }
    if (filterTpm !== 'all') {
  ...
  // Summary stats
  const stats = useMemo(() => {
    if (!data?.tpm_keys) return { total: 0, withTpm: 0, enabled: 0, withEk: 0 };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="relative liquid-glass liquid-glass-shimmer rounded-2xl p-8 w-full max-w-sm text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Enter password to access this page</p>
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-4">
            Demo password: <span className="font-mono text-slate-500 dark:text-slate-400">demo</span>
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-6 max-w-md text-center">
          ...
          <button onClick={loadData} className="px-4 py-2 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg transition-colors text-sm">
            Try Again
          </button>
        </div>
      </div>
    ...
    <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <label
            className={`flex items-center gap-2 px-3 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-lg transition-colors text-sm text-slate-600 dark:text-slate-300 cursor-pointer ${uploading ? 'opacity-50 pointer-events-none' : ''}`}
          >
      ...
      {(collecting || collectLogs.length > 0) && (
        <div className="mb-6 bg-slate-900/80 backdrop-blur-sm border border-slate-700/50 rounded-xl overflow-hidden">
          ...
          >
            ...
            {collecting && (
              <span className="ml-2 text-xs text-orange-400 animate-pulse">● Running</span>
            )}
              ...
              {collectLogs.length === 0 ? (
                ...
                collectLogs.map((line, i) => (
                  <p key={i} className={
                    ...
                    line.includes('RECAP') ? 'text-cyan-400 font-semibold' :
                    line.includes('Imported') ? 'text-emerald-400 font-semibold' :
                    'text-slate-400'
                  }>{line}</p>
                ))
              )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-1 mb-3">
                {data.missing_hosts.slice(0, 10).map((h) => (
                  <p key={h.name} className="text-xs text-slate-400">
                    <span className="text-slate-300 dark:text-slate-300 font-medium">{h.name}</span>
                    <span className="ml-2 text-slate-500">[{h.group}]</span>
                  </p>
                ))}
                {data.missing_count > 10 && (
                  <p className="text-xs text-slate-500">+{data.missing_count - 10} more</p>
                )}
              </div>
              <p className="text-xs text-slate-500">
                Click "Collect Keys" to run the Ansible playbook automatically.
              </p>
            </div>
          </div>
        </div>
          ...
          {importResult.success ? (
            <div>
              ...
              {importResult.errors?.length > 0 && (
                <div className="mt-2">
                  <p className="text-xs text-amber-400 font-medium mb-1">Errors on {importResult.errors.length} host(s):</p>
                  {importResult.errors.map((e, i) => (
                    <p key={i} className="text-xs text-slate-400">{e.hostname}: {e.error}</p>
                  ))}
                </div>
              )}
            </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <select
          ...
          className="px-3 py-2 bg-white/40 dark:bg-white/5 border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm rounded-lg text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/30 outline-none"
        >
        ...
        <select
          value={filterTpm}
          onChange={(e) => setFilterTpm(e.target.value)}
          className="px-3 py-2 bg-white/40 dark:bg-white/5 border border-slate-200/50 dark:border-slate-700/50 backdrop-blur-sm rounded-lg text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500/30 outline-none"
        >
      ...
      <div className="flex items-center justify-between mb-2">
        <p className="text-xs text-slate-500">
          Showing {filtered.length} of {data?.total || 0} hosts
        </p>
      </div>
