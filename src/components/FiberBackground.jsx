...
/* Steel Blue palette for the dark-mode dot field */
const DARK_DOT_COLORS = [
  [77, 169, 240],   // SITE1 blue
  [181, 124, 230],  // SITE2 violet
  [57, 162, 131],   // success teal
  [232, 104, 58],   // warm orange
  [153, 207, 247],  // pale sky
];
...
function FiberBackground() {
  ...
  useEffect(() => {
    ...
    // ── Dark mode: lightweight flowing dot field ─────────────────────────
    // ~70 plain arc fills at 30fps, no gradients per dot, DPR left at 1 —
    // cheap enough to leave running without heating anything up.
    if (isDark) {
      const DOT_COUNT = 70;
      ...
      const dots = Array.from({ length: DOT_COUNT }, () => {
        ...
        return {
          ...
          color: DARK_DOT_COLORS[Math.floor(Math.random() * DARK_DOT_COLORS.length)],
        };
      });
      ...
      const drawDots = () => {
        ...
        for (const d of dots) {
          const [r, g, b] = d.color;
          const pulse = 0.6 + 0.4 * Math.sin(d.pulsePhase);
          ctx.beginPath();
          ctx.fillStyle = `rgba(${r},${g},${b},${d.alpha * pulse})`;
      ...
      if (reducedMotion) {
        drawDots(); // single static frame, no animation loop
        return () => window.removeEventListener('resize', resize);
      }

      let lastFrame = 0;
      const animateDots = (ts) => {
        animationFrameId = requestAnimationFrame(animateDots);
        if (ts - lastFrame < FRAME_MS) return; // 30fps cap
        lastFrame = ts;

        for (const d of dots) {
          d.drift += d.driftSpeed;
          d.pulsePhase += d.pulseSpeed;
          d.x += d.vx + Math.sin(d.drift) * 0.12;
          d.y += d.vy + Math.cos(d.drift * 0.8) * 0.12;
    ...
    // ── Light mode: original aurora (unchanged) ──────────────────────────

    // ── Large aurora orbs ──────────────────────────────────────────────
    const orbDefs = [
      { color: [59, 130, 246],  a: 0.14, r: 320 }, // blue
      { color: [139, 92, 246],  a: 0.12, r: 300 }, // purple
      { color: [6, 182, 212],   a: 0.12, r: 280 }, // cyan
      { color: [16, 185, 129],  a: 0.10, r: 260 }, // emerald
      { color: [249, 115, 22],  a: 0.08, r: 240 }, // orange
    ];
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fiber-bg fixed inset-0 pointer-events-none z-0"
